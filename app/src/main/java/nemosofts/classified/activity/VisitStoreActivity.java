...
public class VisitStoreActivity extends AppCompatActivity {
    ...
    String user_id="", name="",email="", mobile="", profile_images="", registered_on="", member="";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        profile_images = getIntent().getStringExtra("profile_images");
        registered_on = getIntent().getStringExtra("registered_on");
        ...
        helper = new Helper(this, (position, type) -> {
            Intent intent = new Intent(VisitStoreActivity.this, PostDetailsActivity.class);
            intent.putExtra("post_id", arrayList.get(position).getId());
            intent.putExtra("user_id", arrayList.get(position).getUserId());
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            ActivityCompat.startActivity(VisitStoreActivity.this, intent, null);
        });
        ...
        grid = new GridLayoutManager(VisitStoreActivity.this, 1);
        grid.setSpanCount(2);
    ...
    private void getData() {
        if (helper.isNetworkAvailable()) {
            ...
            },helper.getAPIRequest(Callback.METHOD_VISIT_STORE, page, "", "", "", "", user_id, "", "", "","","","","",null, null));
            loadPost.execute();
        } else {
    ...
    public void setAdapter() {
        if(Boolean.FALSE.equals(isScroll)) {
            adapter = new AdapterPost(VisitStoreActivity.this, arrayList, (itemData, position) -> helper.showInterAd(position,""));
    ...
    private void loadNativeAds() {
        if (Boolean.TRUE.equals(Callback.isNativeAd && !Callback.adNetwork.equals(Callback.AD_TYPE_WORTISE)
            ...
            AdLoader.Builder builder = new AdLoader.Builder(VisitStoreActivity.this, Callback.nativeAdID);

            Bundle extras = new Bundle();
            if (ConsentInformation.getInstance(VisitStoreActivity.this).getConsentStatus() != ConsentStatus.PERSONALIZED) {
